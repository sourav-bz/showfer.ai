...
export default function Playground() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const modifiedHtmlContent = data.content.replace('</head>',
          `<style>
          html, body {
            overflow: auto;
  ...
  return (
    <div className="rounded-md w-auto h-full overflow-hidden">
      ...
      <div className="bg-white h-[800px] rounded-md relative">
        {!htmlContent && !error ?
          <div className="flex flex-col items-center justify-center h-full">
            <Image src={"/playground/playground_zero_state.svg"} alt="no_content" height={304} width={304} />
                    ...
                    <div className="h-9 bg-[#E3E4EC] flex items-center justify-between px-6">
                      ...
                      <div className="flex items-center space-x-2">
                        <span className="text-sm">
                          <Image src={"/icons/signal_bars.svg"} alt="signal_bars" height={14} width={14} />
                        </span>
                        <span className="text-sm">
                          <WifiIcon className="w-4 h-4 fill-current" />
                        </span>
                        <span className="text-sm">
                          <Image src={"/icons/battery_100.svg"} alt="battery_100" height={17} width={17} />
                        </span>
                      </div>
                    </div>
                    <iframe
